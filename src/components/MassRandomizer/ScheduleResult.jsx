    ...
    <div className="lg:col-span-8 flex flex-col min-h-0">
      <div className="bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
          <h3 className="font-black text-slate-800">Preview Jadwal: {namaBulan}</h3>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8 bg-slate-200/50">
          ...
          <div id="schedule-export" ref={referensiEkspor} className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 w-full max-w-4xl mx-auto">
              ...
              {jadwalBulanan.map((pekan) => (
                <div key={pekan.id} className="break-inside-avoid">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pl-2">
                    {pekan.sesi.map(sesi => (
                      <div key={sesi.id} className={`border-l-2 border-slate-200 pl-4 ${sesi.layarPenuh ? 'md:col-span-3' : ''}`}>
                        <h4 className="text-xs font-bold text-slate-500 mb-2">{sesi.nama}</h4>
                        <ul className={sesi.petugas.length > 3 ? "grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1" : "space-y-1"}>
